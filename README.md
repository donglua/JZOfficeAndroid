...
| PPTX | 按演示顺序显示幻灯片、固定坐标文字、文本框垂直对齐、倍数/固定行距、首行/悬挂/左右缩进、已保存的自动缩字、矩形图片裁剪与翻转、矩形/椭圆/直线、嵌套组合的位置/缩放/旋转、简单表格、缓存数据的基础折线图、基础主题色与占位符继承 | 忽略动画、其他图表类型、SmartArt、复杂形状、组合整体翻转和 WordArt；不重新计算自动适配；表格行高近似 |
...
PPTX 圆角矩形 `roundRect` 在圆角调整值明确为 `adj = val 0` 时，按等效普通矩形绘制，保留母版/版式底板的填充、描边和层级顺序。默认圆角、非零圆角和其他复杂几何仍不支持；未新增绘制类型或模型字段。
