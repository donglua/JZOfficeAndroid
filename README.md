...
| PPTX | 按演示顺序显示幻灯片、固定坐标文字、文本框垂直对齐、倍数/固定行距、首行/悬挂/左右缩进、已保存的自动缩字、矩形图片裁剪与翻转、矩形/椭圆/直线、嵌套组合的位置/缩放/旋转、简单表格、基础主题色与占位符继承 | 忽略动画、图表、SmartArt、复杂形状、组合整体翻转和 WordArt；不重新计算自动适配；表格行高近似 |
